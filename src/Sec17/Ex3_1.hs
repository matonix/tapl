...
data Term
  ...
  | TmRecord Info [(String, Term)]  -- info, [(label, term)]
  | TmProj Info Term String         -- info, term, label
  | TmTrue Info
  | TmFalse Info
...
printtm ctx (TmVar fi x n) =
  if ctxlength ctx == n then Right (index2name fi ctx x) else Left "[bad index]"
printtm ctx (TmAbs _fi x _ty t1) =
  let (ctx', x') = pickfreshname ctx x
  in  Right ("(λ " ++ x' ++ ". ") <> printtm ctx' t1 <> Right ")"
printtm ctx (TmApp _fi t1 t2) =
  Right "(" <> printtm ctx t1 <> printtm ctx t2 <> Right ")"
printtm ctx (TmRecord _fi rs) =
  let printrs []       = Right "}"
      printrs [(l, t)] = Right l <> Right "= " <> printtm ctx t <> Right "}"
      printrs ((l, t) : ts) =
        Right l <> Right "= " <> printtm ctx t <> Right "," <> printrs ts
  in  Right "{" <> printrs rs
printtm ctx  (TmProj _fi t l) = printtm ctx t <> Right "." <> Right l
printtm _ctx (TmTrue  _fi   ) = Right "true"
printtm _ctx (TmFalse _fi   ) = Right "false"
...
getbinding fi ctx i = if length ctx > i
  then snd $ ctx !! i
    ...
    error' fi
    $  "Variable lookup failure: offset: "
    ++ show i
    ++ ", ctx size: "
    ++ show (length ctx)
...
subtype :: Ty -> Ty -> Bool
subtype tyS tyT = tyS == tyT || case (tyS, tyT) of
  (_              , TyTop          ) -> True
  (TyArr tyS1 tyS2, TyArr tyT1 tyT2) -> subtype tyT1 tyS1 && subtype tyS2 tyT2
  (TyRecord fS, TyRecord fT) ->
    all (\(li, tyTi) -> maybe False (`subtype` tyTi) (lookup li fS)) fT
  _ -> False

typeof :: Context -> Term -> Ty
-- TA-Rcd
typeof ctx (TmRecord _fi fields) = TyRecord $ map (second (typeof ctx)) fields
-- TA-Proj
typeof ctx (TmProj fi t1 l     ) = case typeof ctx t1 of
  (TyRecord fieldtys) -> case lookup l fieldtys of
    Just ty -> ty
    Nothing -> error' fi $ "label " ++ l ++ " not found"
  _ -> error' fi "record type expected"
-- TA-Var
typeof ctx (TmVar fi i _) = getTypeFromContext fi ctx i
-- TA-Abs
typeof ctx (TmAbs _fi x tyT1 t2) =
  let ctx' = addbinding ctx x (VarBind tyT1)
      tyT2 = typeof ctx' t2
  in  TyArr tyT1 tyT2
-- TA-App
typeof ctx (TmApp fi t1 t2) =
...
termShift d t =
  tmmap (\fi c x n -> TmVar fi (x + (if x >= c then d else 0)) (n + d)) 0 t
...
termSubst j s t =
  tmmap (\fi c x n -> if x == j + c then termShift c s else TmVar fi x n) 0 t
...
tmmap onvar = walk
 where
  ...
  walk c  (  TmRecord fi fields) = TmRecord fi (map (second (walk c)) fields)
  walk c  (  TmProj fi t1 l    ) = TmProj fi (walk c t1) l
...
-- E-App1
eval1 ctx (TmApp fi t1 t2) = let t1' = eval1 ctx t1 in TmApp fi t1' t2
-- E-IfTrue
eval1 _ctx (TmIf _ (TmTrue _) t2 _) = t2
-- E-IfFalse
eval1 _ctx (TmIf _ (TmFalse _) _ t3) = t3
-- E-If
eval1 ctx (TmIf fi t1 t2 t3) = let t1' = eval1 ctx t1 in TmIf fi t1' t2 t3
-- No Rule Applies
eval1 _ctx _ = throw NoRuleApplies
