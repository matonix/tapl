module Sec11.SimplyTypedLet where
...
data Term
  ...
  | TmLet Info String Term Term -- info, hint name, term, term
  deriving (Eq, Show)
...
printtm ctx (TmLet _fi x t1 t2) = let (ctx', x') = pickfreshname ctx x in
  Right ("let " ++ x' ++ "=") <> printtm ctx' t1 
    <> Right " in " <> printtm ctx' t2
...
typeof ctx (TmLet _fi x t1 t2) = let
  tyT1 = typeof ctx t1
  ctx' = addbinding ctx x (VarBind tyT1)
  tyT2 = typeof ctx' t2
  in tyT2
...
tmmap onvar = walk where
  ...
  walk c (TmLet fi x t1 t2) = TmLet fi x (walk c t1) (walk c t2)
...
eval1 ctx (TmIf fi t1 t2 t3) =
  ...
  TmIf fi t1' t2 t3  
-- E-LetV
eval1 ctx (TmLet _fi _x v1 t2) | isval ctx v1 =
  termSubstTop v1 t2 
-- E-Let
eval1 ctx (TmLet fi x t1 t2) =
  let t1' = eval1 ctx t1 in
  TmLet fi x t1' t2
-- Otherwise
eval1 _ctx _ =
